        ...
        return (
            <Grid item container xs={12}>
                <Grid className="adminHeader" item lg={3} sm={12}>
                    <Avatar />
                    <Box>
                        <h2 style={{ margin: '3px' }}>Name: {todoStore.adminpage.name} </h2>
                        <p style={{ margin: '3px' }}>Email: {todoStore.adminpage.email}</p>
                    </Box>
                ...
                <Grid className="adminBody" item lg={9} sm={12}>
                    ...
                    {toggleManageAccess ? (
                        <Box>
                            ...
                            {todoStore.adminpage.manage.map((card, index) => {
                                return (
                                    <ManageAccess
                                        key={index}
                                        name={card.name}
                    ...
                    <Box className="adminBody">
                        ...
                        {todoStore.adminpage.pending_items.map((card) => {
                            return (
                                <Item
                                    key={card.itemId}
                                    itemId={card.itemId}
                        ...
                        {todoStore.adminpage.active_items.map((card) => {
                            return (
                                <Item
                                    key={card.itemId}
                                    itemId={card.itemId}
                        ...
                        <Box>
                            {todoStore.adminpage.trans_items.map((card) => {
                                return (
                                    <Item
                                        key={card.itemId}
                                        itemId={card.itemId}
